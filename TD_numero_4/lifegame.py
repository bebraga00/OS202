...
if __name__ == '__main__':
    # guarantee at least two processes
    if(nbp == 1):
        print("Au moins deux processus pour cette topologie !")
        exit(1)
    
    # we only accept a number of processes in the format of one plus a natural power of two
    real_nbp = (2 ** math.floor(math.log2(nbp - 1))) + 1

    if(rank == 0):
        import time
        import sys

        print("Desired number of processes:", nbp)
        print("Actual number of processes:", real_nbp, "(1 for printing and", real_nbp - 1, "for computing)")

        pg.init()
        dico_patterns = { # dimension and pattern in a tuple
        ...
        lines_each_rank = init_pattern[0][0]
        columns_each_rank = init_pattern[0][1]
        cut_horizontal = True
        horizontal_cuts = 0
        vertical_cuts = 0
        
        # find how many cuts we make with alternate direction splitting
        number_of_cuts = math.floor(math.log2(nbp - 1))
        if(number_of_cuts == 0):
            new_nbp = 1
        else:
            new_nbp = number_of_cuts * 2
        ...
        for i in range(number_of_cuts):
            ...
            cut_horizontal = not(cut_horizontal)

        print("Horizontal cuts:", horizontal_cuts)
        print("Vertical cuts", vertical_cuts)

        grid = Grille(*init_pattern)
        appli = App((resx, resy), grid)

        coordinates = np.full(((horizontal_cuts + 1), (vertical_cuts + 1) * 2), [0, 0])
        ...
        for i in range(horizontal_cuts + 1):
            for j in range(vertical_cuts + 1): 
                if(i == 0):
                    number_of_lines = lines_each_rank + 1
                    print("from line 0 to", (number_of_lines - 1))
                elif(i == horizontal_cuts):
                    print("from line", (i - 1 + 1) * lines_each_rank,"to line", (i + 1) * lines_each_rank - 1)
                    number_of_lines = lines_each_rank + 1
                else:
                    number_of_lines = lines_each_rank + 2
                if(j == 0):
                    number_of_columns = columns_each_rank + 1
                elif(j == horizontal_cuts):
                    number_of_columns = columns_each_rank + 1
                else:
                    number_of_columns = columns_each_rank + 2
                coordinates[i][j * 2] = number_of_lines
                coordinates[i][j * 2 + 1] = number_of_columns
                # print("from line", i * , "to", 1, "and from column", 1, "to", 1)

        print(coordinates)
                

        # print(init_pattern[0][0], init_pattern[0][1], "|", init_pattern[1])
        # print("---")
        # print(*init_pattern)
